...
const Home = () => {
  ...
  return (
    <MathJaxContext config={config}>
      <section className={`flex flex-col ${styles.paddingX} justify-center items-center h-[80vh]`}>
        <div className='mt-5 flex flex-row w-full justify-around items-center p-1'>
          <div className='flex flex-col justify-around items-center'>
            <h1 className='font-bold'>Section Name</h1>
            <h1>Q No: {questionIndex + 1}/{totalQuestions}</h1>
          </div>
          <div className='flex flex-col justify-around items-center'>
            <h1>Student Name</h1>
            <h1>Test Roll No: 241234</h1>
          </div>
          <div className='flex flex-col justify-around items-center'>
            <h1>Timer: 01:30:55</h1>
            <h1 className='font-bold'>Negative Marking: YES</h1>
          </div>
        </div>
        ...
        <div id='features' className='mt-[-1.0rem] scrollable-element flex gap-1 md:flex-row flex-col items-center justify-between h-[100%] p-[2rem] w-[100vw]'>
          <MathJaxContext>
            <div className="flex-[3] border-solid border-2 border-white w-full h-[100%] overflow-auto">
              ...
              <div className='flex flex-col justify-start items-start gap-4 m-4'>
                <div className='flex justify-around items-start w-full text-black'>
                  <button
                    className={`bg-white px-4 ${!selectedOptions[questionIndex] ? 'disabled' : ''}`}
                    onClick={handleReset}
                    disabled={!selectedOptions[questionIndex]}
                  >
                    Reset
                  </button>
                  <button className='bg-white px-4' onClick={handleBackClick} disabled={questionIndex <= 0}>Back</button>
                  <button className='bg-white px-4' onClick={handleNextClick} disabled={questionData && questionIndex >= questionData.length - 1}>Next</button>
                  <button className='bg-white px-4' onClick={handleReviewClick}>Review</button>
                </div>
              </div>
            </div>
          </MathJaxContext>
          ...
          {/* Review Questions */}
          <div className='flex-[1] border-solid border-2 border-white w-full h-[100%]'>
